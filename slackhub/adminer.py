...
def add_admin(message, admin):
    ...
    """
    save_admin(admin)
    return 'Admin [*' + admin + '*] has been added.'
...
def remove_admin(message, admin):
    ...
    """
    delete_admin(admin)
    return 'Admin [*' + admin + '*] has been removed.'
...
def list_channel_actions(message, channel_id, action):
    ...
    if data is not None:
        ...
        return channel_id + ' settings for [*' + action + '*] are as follows: ' + reply
    else:
...
def add_channel_actions(message, channel_id, action, target):
    ...
    return '#' + details.get('username') + ' subscribed to ' + action + ' [*' + target + '*]'
...
def add_channel_repo_actions(message, channel_id, name, action, target):
    ...
    return '#' + repo_config[0].get('username') + ' subscribed to ' + action + ' [*'\
           + target + '*] in repo *' + name + '*'
...
def add_channel_repos(message, channel_id, name):
    ...
    return '#' + repo_config[0].get('username') + ' subscribed to repository ' + ' [*' + name + '*]'
...
def remove_channel_actions(message, channel_id, action, target):
    ...
    return '#' + details.get('username') + ' unsubscribed from ' + action + ' [*' + target + '*]'
...
def remove_channel_repo_actions(message, channel_id, name, action, target):
    ...
    return '#' + repo_config[0].get('username') + ' unsubscribed from ' + action\
           + ' [*' + target + '*] in repo *' + name + '*'
...
def remove_channel_repos(message, channel_id, name):
    ...
    return '#' + repo_config[0].get('username') + ' unsubscribed from repository '\
           + ' [*' + name + '*]'
...
def disable_notifications(message, channel_id, target):
    ...
    return '#' + details.get('username') + ' disabled [*' + target\
...
def disable_repo_notifications(message, channel_id, name, target):
    ...
    return '#' + repo_config[0].get('username') + ' disabled [*' + target + '*] on repo '\
...
def enable_notifications(message, channel_id, target):
    ...
    return '#' + details.get('username') + ' enabled [*' + target\
...
def enable_repo_notifications(message, channel_id, name, target):
    ...
    return '#' + repo_config[0].get('username') + ' enabled [*' + target + '*] on repo '\
